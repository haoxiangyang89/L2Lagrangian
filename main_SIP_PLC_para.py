...
def build_ls_lb_problem(x_value, L_value, cutList, x_tilde, prob_lb=-10000, prob_ub=10000):
    # input: 
    ...
    # set up the objective function with Lagrangian penalty term
    lb_prob.setObjective(-gp.quicksum(pi[i] * x_tilde[i] for i in range(J_len)) - theta, GRB.MINIMIZE)
...
def update_ls_lb_problem(lb_prob, J_len, x_tilde, cutList, update_ind_range):
    ...
    # set up the objective function with Lagrangian penalty term
    lb_prob.setObjective(-gp.quicksum(lb_prob.getVarByName("pi[{}]".format(i)) * x_tilde[i] for i in range(J_len)) - lb_prob.getVarByName("theta"), GRB.MINIMIZE)
    lb_prob.update()
    return lb_prob

# Build the level set lower bound problem
def build_next_pi_problem(level, alpha, x_value, L_value, cutList, x_tilde, prob_lb=-10000, prob_ub=10000):
    # input: 
    # x_value - the optimal solution of the master problem, pi_value - the Lagrangian dual multipliers,
    # L_value - the optimal value of Lagrangian function evaluated at x_value,
    # cutList - the list of cuts generated for the inner minimization problem so far
    ...
    next_pi_prob.addConstrs((gp.quicksum(cutList[j][0][i] * pi[i] for i in range(J_len)) + cutList[j][1] >= theta
                                for j in range(len(cutList))), name = "cuts")
    next_pi_prob.addConstr(-alpha * (gp.quicksum(pi[i] * x_tilde[i] for i in range(J_len)) + theta) + 
...
def update_next_pi_problem(next_pi_prob, J_len, cutList, update_ind_range, alpha, x_value, L_value, pi_bar_value, level, x_tilde):
    ...
    next_pi_prob.addConstr(-alpha * (gp.quicksum(next_pi_prob.getVarByName("pi[{}]".format(i)) * x_tilde[i] for i in range(J_len)) + next_pi_prob.getVarByName("theta")) + 
...
def obtain_alpha_bounds_opt(pi_list, L_value, x_value, v_underbar, V_list, x_tilde, prob_lb=-10000):
    # input: 
    ...
    alpha_prob.addConstrs((alpha_obj <= alpha * (-(np.inner(pi_list[j], x_tilde) + V_list[j]) - v_underbar) + \
...
def obtain_alpha_bounds(pi_list, L_value, x_value, v_underbar, V_list, x_tilde):
...
def solve_lag_dual(o, ho, T, W, c, y_option, x_value, L_value, lambda_level, mu_level, x_tilde, tol = 1e-2, cutList = [], sub_lb = -10000, sub_ub = 100000):
    # input: 
    # o - index of the scenario, ho - the right-hand side of the structural constraints,
    # T - the coefficient matrix of x variables in sub, W - the coefficient matrix of y variables in sub,
    # c - the objective function coefficients, y_option - the type of the decision variables: binary or integer,
    ...
    lb_prob = build_ls_lb_problem(x_value, L_value, cutList, x_tilde)
    # set up the auxiliary problem to find the next pi_value
    level = sub_ub
    next_pi_prob = build_next_pi_problem(level, alpha, x_value, L_value, cutList, x_tilde)
    ...
    while cont_bool:
        ...
        # update the alpha
        alpha_max, alpha_min, Delta = obtain_alpha_bounds(pi_list, L_value, x_value, lb_prob.ObjVal, V_list, x_tilde)
        if counter == 0:
            alpha = (alpha_max + alpha_min) / 2
        else:
        ...
        if Delta < tol:
            ...
            if counter > 200:
                ...
                counter += 1
                # update the level
                v_bar_list = [-alpha * (np.inner(pi_list[pi_j_ind], x_tilde) + V_list[pi_j_ind]) + (1 - alpha) * (L_value - np.inner(pi_list[pi_j_ind], x_value) - V_list[pi_j_ind]) for pi_j_ind in range(len(pi_list))]
...
def sub_routine(o, h, T, W, c, y_option, x_value, lambda_level, mu_level, x_tilde, tol = 1e-2, cut_Dict = {}):
    # input: 
    ...
    pi_value_o, v_value_o, cutList_o = solve_lag_dual(o, h[o], T, W, c, y_option, x_value, L_value, lambda_level, mu_level, x_tilde, tol, cut_Dict[o])
...
if __name__ == "__main__":
    ...
    while iter_bool:
        ...
        sub_opt_results = pool.map(partial(sub_routine, h=h, T=T, W=W, c=c, y_option=y_option, x_value=x_value, lambda_level=lambda_level, 
